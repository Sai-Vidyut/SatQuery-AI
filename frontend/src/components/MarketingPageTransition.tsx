...
const MARKETING_PATHS = new Set(["/home", "/tutorial", "/credits"]);
...
export function MarketingPageTransition({ children }: Props) {
  ...
  return (
    <>
      ...
      {isMarketing && showLoader ? (
        <div className="marketing-transition-loader marketing-transition-loader--visible" aria-hidden="true">
          <SolarLoader size={30} speed={2} />
        </div>
      ) : null}
    </>
  );
